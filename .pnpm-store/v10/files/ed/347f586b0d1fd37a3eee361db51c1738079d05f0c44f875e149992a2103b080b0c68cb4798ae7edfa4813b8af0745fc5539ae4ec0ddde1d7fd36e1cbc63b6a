        ...
        addKey: (keyInfo) => {
            ...
            if (warnOnConflicts) {
                ...
                if (prev !== undefined && prev !== defaultValue && !isGenericFallback) {
                    const msg = `Key "${uniqueKey}" has conflicting default values:\n` +
                        `  "${prev}" (first seen)\n` +
...
export { ConflictError, createPluginContext, initializePlugins };
